resource "aws_instance" "JkgsD" {
  ...
  tags_all = {
    Name         = "external-worker-worker-eu-we1-prod"
    client       = "cycloid"
    "cycloid.io" = aws_cloudfront_distribution.cycloidio_website_cloudfront_prod.enabled
    env          = "prod"
    project      = "external-worker"
    role         = "worker"
  }
