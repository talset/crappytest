resource "aws_instance" "NlYVE" {
  ...
  instance_type        = var.aws_instance_NlYVE_instance_type
  key_name             = "cycloid"
  metadata_options {
...
resource "aws_instance" "pzcgR" {
  ...
  iam_instance_profile = "cycloid_profile-front-cycloidio-website-prod"
  instance_type        = var.aws_instance_pzcgR_instance_type
  key_name             = "cycloid"
  metadata_options {
    http_endpoint               = "enabled"
    http_put_response_hop_limit = 1
...
resource "aws_launch_configuration" "EgLFd" {
  ebs_block_device {
    delete_on_termination = true
...
resource "aws_launch_configuration" "ElICu" {
  enable_monitoring = true
...
resource "aws_launch_configuration" "NLbdC" {
  ebs_block_device {
    delete_on_termination = true
    device_name           = "/dev/xvdf"
    volume_size           = 150
...
resource "aws_launch_configuration" "RsBSK" {
  ebs_block_device {
    delete_on_termination = true
    device_name           = "/dev/xvdf"
    volume_size           = 150
...
resource "aws_launch_configuration" "SJwWB" {
  ebs_block_device {
    delete_on_termination = true
    device_name           = "/dev/xvdf"
    volume_size           = 150
...
resource "aws_launch_template" "EVTwI" {
  tags = {
...
resource "aws_launch_template" "TOcqe" {
  tags = {
