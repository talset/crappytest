...
module "external-worker" {
  ...
  bastion_sg_allow         = data.terraform_remote_state.infrastructure.outputs.prod_bastion_sg_allow
  ...
  metrics_sg_allow        = data.terraform_remote_state.prometheus.outputs.prod_metrics_sg_allow

  #. public_subnets_ids (required, array):
  #+ Amazon public subnets IDs on which create each components.
  public_subnets_ids      = data.terraform_remote_state.infrastructure.outputs.prod_public_subnets
  ...
  #. worker_spot_request_type (optional): ""
  #+ The spot instance request type to use. Can be either `one-time`, `persistent` or left undefined.

  #. worker_spot_price (optional): ""
  #+ The maximum hourly price you're willing to pay for the Spot Instances. Linked to instance type. Undefined by default.
}
