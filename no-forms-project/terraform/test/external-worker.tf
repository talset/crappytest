...
module "external-worker" {
  ...
  metrics_sg_allow        = data.terraform_remote_state.prometheus.outputs.test_metrics_sg_allow

  #. public_subnets_ids (required, array):
  #+ Amazon public subnets IDs on which create each components.
  public_subnets_ids      = data.terraform_remote_state.infrastructure.outputs.test_public_subnets
