resource "aws_instance" "HNhvD" {
  ...
  instance_type                        = var.aws_instance_hnhvd_instance_type
  key_name                             = aws_db_instance.cycloid_cycloidio_website_rds_staging.username
  metadata_options {
...
resource "aws_instance" "OMXEj" {
  ...
  instance_type                        = var.aws_instance_omxej_instance_type
  key_name                             = aws_db_instance.cycloid_cycloidio_website_rds_staging.username
  metadata_options {
    http_endpoint               = "enabled"
    http_put_response_hop_limit = 1
...
resource "aws_instance" "cSiDi" {
  ...
  ami                         = "ami-0c58738a674b21440"
  associate_public_ip_address = true
  availability_zone           = aws_db_instance.cycloid_cycloidio_website_rds_staging.availability_zone
  cpu_core_count              = 4
  cpu_threads_per_core        = 2
  ebs_block_device {
    delete_on_termination = true
  ...
  iam_instance_profile                 = "profile-worker-external-worker-prod"
  instance_initiated_shutdown_behavior = "terminate"
  instance_type                        = var.aws_instance_csidi_instance_type
  key_name                             = "cycloid"
  metadata_options {
    http_endpoint               = "enabled"
    http_put_response_hop_limit = 1
...
resource "aws_instance" "calUp" {
  ...
  iam_instance_profile                 = "profile-worker-external-worker-prod"
  instance_initiated_shutdown_behavior = "terminate"
  instance_type                        = var.aws_instance_calup_instance_type
  key_name                             = "cycloid"
  metadata_options {
    http_endpoint               = "enabled"
    http_put_response_hop_limit = 1
...
resource "aws_instance" "dedicated_env_cy_instances_0_gl_cc_7" {
  ...
  tenancy                = "default"
  vpc_security_group_ids = ["sg-12eee674", "sg-0d96d0c0dcebb46b1"]
}
...
resource "aws_instance" "monitoring_prometheus_eu_we1_infra" {
  ...
  root_block_device {
    tags = {
      Name         = "monitoring-prometheus-eu-we1-infra"
      customer     = aws_db_instance.cycloid_cycloidio_website_rds_staging.username
      "cycloid.io" = aws_db_instance.cycloid_cycloidio_website_rds_prod.multi_az
      env          = "infra"
      project      = "monitoring"
      role         = "prometheus"
